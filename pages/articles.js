...
const MovingImage = ({ title, img, link }) => {
  ...
  return (
    ...
    >
      <h2 className="capitalize font-semibold text-xl hover:underline md:!hidden">
        {title}
      </h2>
      <FrameImage
        style={{ x: x, y: y }}
  ...
  return (
    <li className="col-span-1 w-full p-4 bg-light dark:bg-dark border border-solid border-dark dark:border-light rounded-2xl border-r-4 border-b-4 ">
      ...
      <Link href={link} target="_blank">
        <h2
          className="my-2 mt-4 capitalize font-bold text-2xl hover:underline dark:text-light
        xs:text-lg"
        >
          {title}
        </h2>
      ...
      <main className=" w-full flex flex-col items-center justify-center overflow-hidden dark:text-light">
        <Layout className="pt-16">
          <AnimatedText
            ...
            className="mb-16 lg:!text-center lg:!text-6xl md:!text-5xl sm:!text-5xl xs:text-3xl"
          />
